...
const createPayload = (progress, extensionUrl) => {
  ...
  if (progress.error) {
    msg.attachments.push({
      color: '#F35A00',
      fallback: `TFS to Auth0 Deployment failed: ${progress.error.message}`,
      text: `TFS to Auth0 Deployment failed ${details}`,
      fields: [
    ...
    msg.attachments.push({
      color: '#7CD197',
      fallback: 'TFS to Auth0 Deployment',
      text: `TFS to Auth0 Deployment ${details}`,
      fields
    });
  }
